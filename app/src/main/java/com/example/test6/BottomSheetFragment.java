...
public class BottomSheetFragment extends BottomSheetDialogFragment {
    ...
    com.example.test6.BSAdapter bs_adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        bs_adapter = new com.example.test6.BSAdapter(this,hawker_name);
    ...
    private void insertFav() {
        ...
        filepath.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            ...
            public void onSuccess(Uri uri) {
                String c_uri = uri.toString();

                DocumentReference docRef = userdb.collection("CUSTOMER").document(username).collection("favourites").document(hawker_name);
                Map<String, String> items = new HashMap<>();
                items.put("profile", c_uri);
                docRef.set(items).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
//                        Toast.makeText(getContext(), "Vendor added to favourites", Toast.LENGTH_SHORT).show();
                    }
                });
    ...
    private void remove_fav() {
        userdb.collection("CUSTOMER").document(username).collection("favourites").document(hawker_name)
                .delete()
            ...
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                ...
                for (DocumentSnapshot doc : value) {
                    if (error != null) {
                        ...
                        if(doc.getId().toString().equals(hawker_name)){
                            bs_cb1.setChecked(true);
                            break;
                        }
                    }
                }
